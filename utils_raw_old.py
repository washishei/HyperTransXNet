...
def open_file(dataset):
    ...
    if ext == '.mat':
        ...
        #return TIFF.open(dataset, mode='r')
...
def metrics(prediction, target, ignored_labels=[], n_classes=None):
    ...
    total = np.sum(cm[1:,:])
    accuracy = sum([cm[x][x] for x in range(1, len(cm))])
    ...
    Recalls = np.zeros(len(cm))
    for i in range(len(cm)):
        if np.count_nonzero(target == i) != 0:
            Precisions[i] = cm[i, i] / np.count_nonzero(target == i)
        else:
            Precisions[i] = np.NaN
        if sum(cm[:, 1]) != 0:
            Recalls[i] = cm[i, i] / sum(cm[:, 1])
        else:
            Recalls[i] = np.NaN
...
def show_results_new(results, vis, label_values=None, aggregated=False, df_column_entry_dict=None):
    text = ""

    if aggregated:
        accuracies = [r["Accuracy"] for r in results]
        average_accuracies = [r["Average Accuracy"] for r in results]
        ...
        df_column_entry_dict['Confusion matrix'] = cm = results["Confusion matrix"]
        df_column_entry_dict['Overall Accuracy [%]'] = accuracy = results["Accuracy"]
        df_column_entry_dict['Average Accuracy [%]'] = average_accuracy = results["Average Accuracy"]
        F1scores = results["F1 scores"]
        Precisions = results["Precisions"]
        Recalls = results["Recalls"]

        df_column_entry_dict['Kappa Coefficient'] = kappa = results["Kappa"]
        for label, score in zip(label_values, Precisions):
            df_column_entry_dict['Precision '+label] = score
        for label, score in zip(label_values, Recalls):
            df_column_entry_dict['Recall '+label] = score
    ...
    if aggregated:
        text += ("Average Accuracy: {:.03f} +- {:.03f}\n".format(np.mean(average_accuracies),
                                                         np.std(average_accuracies)))
    else:
    ...
    if aggregated:
        ...
        for label, score in zip(label_values, F1scores):
            text += "\t{}: {:.03f}\n".format(label, score)
            df_column_entry_dict['F1 '+label] = score
    text += "---\n"

    if aggregated:
        text += ("Kappa Coefficient: {:.03f} +- {:.03f}\n".format(np.mean(kappas),
                                                      np.std(kappas)))
    else:
        text += "Kappa Coefficient: {:.03f}\n".format(kappa)
