...
from vit_pytorch.conformer import Conformer as ConformerViT
...
# from vit_pytorch.AVT import AvT
from vit_pytorch.AVT_new import AvT
from vit_pytorch.TAVT import TAvT
from vit_pytorch.ETM import ETM
from vit_pytorch.conformer_new import Conformer
from vit_pytorch.conformerNA import ACTransformerNA
from vit_pytorch.ACTransformer import ACTransformer
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = Conformer(patch_size=15, channel_ratio=8, embed_dim=384, depth=12,
                      num_heads=8, mlp_ratio=8, qkv_bias=True, num_classes=n_classes, in_chans=n_bands)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = ACTransformerNA(patch_size=15, channel_ratio=8, embed_dim=384, depth=12,
                      num_heads=8, mlp_ratio=8, qkv_bias=True, num_classes=n_classes, in_chans=n_bands)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = ACTransformer(patch_size=15, channel_ratio=6, embed_dim=384, depth=12,
                      num_heads=8, mlp_ratio=6, qkv_bias=True, num_classes=n_classes, in_chans=n_bands)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("epoch", 100)
        kwargs.setdefault("batch_size", 100)
    elif name == "conformervit":
        kwargs["supervision"] = "con"
        kwargs.setdefault("patch_size", 16)
        center_pixel = True
        model = ConformerViT(patch_size=16, num_classes=n_classes, in_chans=n_bands)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = ETM(num_classes=n_classes, channels=n_bands,)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        segment_dim = [8, 8, 4, 4]
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [196, 196, 25, 256]## for IN 368, for GRSS 256, for PU 168, for KSC 320 for XA 480
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            if supervision == "full":
                ...
                outs = net(data)
                output, rec, final_out = outs
                loss5 = nn.KLDivLoss(reduction='batchmean')
                loss6 = loss5(F.softmax(output, dim=1), F.softmax(rec, dim=1))
                # loss5 = F.pairwise_distance(output, rec)
                # loss5 = torch.cosine_similarity(output, rec, dim=1)
                # loss6 = torch.max(loss5) - 0.05 * loss6
                loss = criterion(output, target) + criterion(rec, target) + 0.5 * criterion(final_out, target) - 0.005 * loss6
