...
from RL.RLEhancedViT import FinalRLSST as RLT
from RL.RLEhancedViT import *
# from RL.RLEhancedViT import PPOPolicy
...
def get_model(name, **kwargs):
    ...
    #     )
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = RLT(in_channels=n_bands, num_classes=n_classes, spatial_dim=15, spectral_dim=128)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
...
def train(
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            optimizer.zero_grad()
            if supervision == "full":
                output, values, dists, attn_data = net(data)
                # print(output.shape)
                s_map, sp_weights = attn_data
                reward = adaptive_reward(output.detach(), target, s_map, sp_weights)
                value_loss = sum([value_criterion(v.squeeze(), reward) for v in values])
                policy_loss = 0
                for dist in dists:  # dists是(s_dist, sp_dist)两个Normal对象
                    # 调整reward维度以匹配分布均值
                    reward_reshaped = reward.view(-1, *([1] * (dist.mean.dim() - 1)))
                ...
                loss = cls_loss + 0.4 * value_loss + 0.6 * policy_loss
...
def compute_reward(pred, labels):
    acc = (pred.argmax(1) == labels).float()
    conf = pred.softmax(1).max(1)[0]
    return acc + 0.3 * conf

def adaptive_reward(pred, target, s_map, sp_weights):
    # 分类准确率
    acc = (pred.argmax(1) == target).float()
