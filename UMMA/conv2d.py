...
class Conv3(torch.nn.Module):
    def __init__(self, input_channels, flatten=True):
        ...
        self.layer1 = nn.Sequential(collections.OrderedDict([
          ('conv',    nn.Conv3d(1, 8, (3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))),
          ('bn',      nn.BatchNorm3d(8)),
          ('relu',    nn.ReLU()),
        ...
        self.layer3 = nn.Sequential(collections.OrderedDict([
          ('conv',    nn.Conv3d(16, 32, (3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1))),
          ('bn',      nn.BatchNorm3d(32)),
          ('relu',    nn.ReLU()),
          ('avgpool', nn.Conv3d(32, 32, (3, 3, 3), stride=(2, 1, 1), padding=(1, 1, 1))),
        ]))

        self.layer4 = nn.Sequential(collections.OrderedDict([
          ('conv',    nn.Conv2d(800, 512, kernel_size=3, stride=1, padding=1, bias=False)),
          ('bn',      nn.BatchNorm2d(512)),
          ('relu',    nn.ReLU()),
          #('avgpool', nn.AvgPool2d(kernel_size=4))
          # ('glbpool', nn.AdaptiveAvgPool2d(1))
...
class TESTEtAl(nn.Module):
    ...
    def __init__(self, input_channels, dim, patch_size=15, pool_size=None):
        super(TESTEtAl, self).__init__()
        self.input_channels = input_channels
        self.patch_size = patch_size
        self.aux_loss_weight = 1
        m = self.patch_size * self.patch_size
        ...
        self.feature = ConvEtAl(input_channels, flatten=False)
        # self.feature = Conv3(input_channels, flatten=False)
        # self.features_sizes = self._get_sizes()
        self.conv_final = nn.Sequential(collections.OrderedDict([
          ('conv',    nn.Conv1d(225, dim, kernel_size=3, stride=1, padding=1, bias=False)),
          # ('bn',      nn.BatchNorm2d(512)),
          # ('relu',    nn.ReLU()),
          # ('avgpool', nn.AvgPool2d(kernel_size=4))
          # ('glbpool', nn.AdaptiveAvgPool2d(1))
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        # x = self.conv_final(x)
        # print("shape of the conv2d_output；",x.shape)
        # x = self.classifier(x)
        # print(x.shape)
        # print(x.size())
        return x
...
if __name__ == '__main__':
    ...
    model = TESTEtAl(200, 1024, patch_size=15)
