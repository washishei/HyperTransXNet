#定义三个骨干网络：ConvTransformerBackbone、ConvBackbone、ConvHRLRFullResSelfAttTransformerBackboneRevised
#proj()：MaskedConv1D(c0, c1, 1)
import torch
from torch import nn
from torch.nn import functional as F
from einops import repeat

from UMMA.blocks import (get_sinusoid_encoding, TransformerBlock, MaskedConv1D, MutilModelTransformerBlock,ConvBlock, LayerNorm, MoEBlock)
...
class ConvTransformerBackbone(nn.Module):
    ...
    def __init__(
        ...
        arch = (3, 3, 5),      # (#convs, #stem transformers, #branch transformers)
...
class ConvHRLRFullResSelfAttTransformerBackboneRevised(nn.Module):
    ...
    def __init__(    #初始化函数
        ...
        n_embd_ks = 3,         # conv kernel size of the embedding network 这个不确定，其他遵循原代码
        max_len=768,           # max sequence length
        arch = (1, 1, 1),      # (#convs, #stem transformers, #branch transformers) fpn_level = arch[2]+1
        mha_win_size = [-1]*2, # size of local window for mha
        scale_factor = 2,      # dowsampling rate for the branch
        with_ln = False,       # if to attach layernorm after conv
        attn_pdrop = 0.3,      # dropout rate for the attention map
        proj_pdrop = 0.0,      # dropout rate for the projection / MLP
        path_pdrop = 0.0,      # droput rate for drop path
        use_abs_pe = False,    # use absolute position embedding
        use_rel_pe = False,    # use relative position embedding
        ...
        self.use_rel_pe = use_rel_pe
        self.use_time_weight = use_time_weight
        self.moe_tf = MoEBlock(dim=768, heads=4, num_experts=32, num_experts_per_token=2)
        # feature projection 进行序列投影 通过Maskedconv1D
        self.n_in = n_in
        if isinstance(n_in, (list, tuple)):
            assert isinstance(n_embd, (list, tuple)) and len(n_in) == len(n_embd)
            self.proj = nn.ModuleList([
    ...
    def forward(self, x, norm_x, reco_x, mask):         #输入段
        # x: batch size, feature channel, sequence length,
        # mask: batch size, 1, sequence length (bool)
        ...
        # embedding network 对x，norm_x,reco_x遍历上面的嵌入网络
        for idx in range(len(self.embd)):
            x, mask = self.embd[idx](x, mask)
            x = self.relu(self.embd_norm[idx](x))
        ...
        # x = self.resselfattention(x,mask, x_k=reco_x, mask_k=mask, x_v=x, mask_v=mask)[0] #使用MutilModelTransformerBlock
        #
        # # stem transformer
        # for idx in range(len(self.stem)):
        #     x, mask = self.stem[idx](x, mask)
