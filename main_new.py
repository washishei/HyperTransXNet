...
from datasets_new import get_dataset, HyperX, open_file, DATASETS_CONFIG
from models_new import get_model, train, test, save_model,t_SNE
import matplotlib.pyplot as plt
# from control import check_gpu_mem_usedRate
#test flops params#
...
#color_gt = color_gt[100:400, 1050:1350]
plt.imshow(color_gt)
plt.axis('off')
plt.savefig('Houston_gt.png',bbox_inches='tight', pad_inches=0)
plt.show()
exit()
...
for run in range(N_RUNS):
    if TRAIN_GT is not None and TEST_GT is not None:
        ...
        #print(train_gt.shape,test_gt.shape)#same with gt.shape
    print(
    ...
    if MODEL == "SVM_grid":
        ...
        with torch.no_grad():
            ...
            input = input.to(hyperparams["device"])
            #flops,params = profile(model.to(hyperparams["device"]), (input,))
        ...
        if t_sne:
            ...
            model.load_state_dict(torch.load("checkpoints/fever_net/IndianPines/Indian_DCTN_300.pth"))
            ...
            # model.load_state_dict(torch.load("checkpoints/ssft_tnet/PaviaU/PaviaU_SSFTT.pth"))
            train_dataset = HyperX(img, train_gt, **hyperparams)
            train_loader = data.DataLoader(
                train_dataset,
                batch_size=hyperparams["batch_size"],
    ...
    ###保存预测图像
    ## saveImage = color_prediction[0:300, 100:400]
    # saveImage = color_prediction
    # plt.imshow(saveImage)
    # plt.axis('off')
    # plt.savefig('squares_plot.png', bbox_inches='tight', pad_inches=0)
    # plt.show()
    # exit()
    ###
    display_predictions(
