    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            if supervision == "full":
                ...
                output, outs, final_out = net(data)
                loss1 = criterion(output, target)
                loss2 = criterion(outs, target)
                loss3 = criterion(final_out, target)
                #loss3 = torch.cosine_similarity(output, outs, dim=1)
                # loss4 = torch.max(loss3)
                ## oushi
                # loss3 = F.pairwise_distance(output, outs)
                ### diancheng
                loss5 = nn.KLDivLoss(reduction='batchmean')
                loss6 = loss5(F.softmax(output, dim=1), F.softmax(outs, dim=1))
                # loss4 = torch.max(loss3)
                #loss = loss1 * 0.5 + loss2 * 0.5 - 0.005 * loss4
                loss = loss1 * 0.5 + loss2 * 0.5 + 0.005 * loss6 + loss3
...
def test(net, img, hyperparams):
    ...
    for batch in tqdm(
        ...
        with torch.no_grad():
            ...
            data = data.to(device)
            output, conv_out, final_out = net(data)
            ...
            if patch_size == 1 or center_pixel:
                ...
                final_out = np.transpose(final_out.numpy(), (0, 2, 3, 1))
            for (x, y, w, h), out in zip(indices, final_out):
                if center_pixel:
...
def val(net, data_loader, device="cpu", supervision="full"):
    ...
    for batch_idx, (data, target) in enumerate(data_loader):
        with torch.no_grad():
            ...
            _, final_out = torch.max(final_out, dim=1)
            for out, pred in zip(final_out.view(-1), target.view(-1)):
