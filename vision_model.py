...
def show_feature_map(img_src, conv_feature):
    ...
    for i in range(3):
        ...
        point_color = (0, 0, 255)
        #superimg = heatmap * 0.3 + np.array(rgb)[:, :, ::-1]  # 图像叠加，注意翻转通道，cv用的是bgr
        rgb = np.array(rgb)[:, :, ::-1]
        superimg = cv2.addWeighted(rgb, 1, heatmap, 0.3, 0)
...
def test(net, img, hyperparams):
    ...
    hook_ref_1 = SaveConvFeatures(net.feature.layer1)
    hook_ref_2 = SaveConvFeatures(net.feature.layer2)
    hook_ref_3 = SaveConvFeatures(net.feature.layer3)
    ...
    for batch in tqdm(
        ...
        with torch.no_grad():
            ...
            conv_features.append(hook_ref_2.features)
            conv_features.append(hook_ref_3.features)
            hook_ref_1.remove()
            hook_ref_2.remove()
            hook_ref_3.remove()
            show_feature_map(data, conv_features)
