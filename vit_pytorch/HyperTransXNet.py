...
class SpectralGlobalUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_spatial = rearrange(x, 'b c h w -> (h w) b c')
...
class HyperSpectralClassifier(nn.Module):
    def __init__(self, input_dim, num_classes, reduce_dim=30, num_blocks=4):
        ...
        #             nn.Conv2d(current_dim, current_dim * 2, 3, stride=1, padding=1),  # 下采样并扩展通道
        ...
        self.blocks1 = nn.Sequential(
            SpectralHybridBlock(current_dim),
            # nn.Conv2d(current_dim, current_dim, 3, stride=1, padding=1),  # 下采样并扩展通道
        ...
        self.blocks3 = nn.Sequential(
            SpectralHybridBlock(current_dim * 2),
            # nn.Conv2d(current_dim * 2, current_dim * 2, 3, stride=1, padding=1),  # 下采样并扩展通道
            # nn.BatchNorm2d(current_dim * 2),
            # nn.ReLU(),
            # nn.AvgPool2d(kernel_size=2, stride=2)
        )
        self.blocks4 = nn.Sequential(
            SpectralHybridBlock(current_dim * 2),
            nn.Conv2d(current_dim * 2, current_dim * 4, 3, stride=1, padding=1),  # 下采样并扩展通道
        ...
        self.head = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(current_dim*4, num_classes)
        )
...
class SpectralHybridBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # 动态融合（保持维度一致）
        fused = self.proj(local_feat + global_feat) #+ global_feat
