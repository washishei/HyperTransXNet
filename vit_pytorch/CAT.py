...
class OverlapPatchEmbed(nn.Module):
    def __init__(self, in_c=3, embed_dim=48, bias=False):
        ...
        self.proj1 = nn.Conv2d(in_c, embed_dim, kernel_size=1, stride=1, padding=0, bias=bias, groups=1)
        self.proj2 = nn.Conv2d(embed_dim, embed_dim, kernel_size=3, stride=1, padding=1, bias=bias, groups=embed_dim)
    ...
    def forward(self, x):
        x = self.Act1(self.proj1(self.BN(x)))
...
class CAT(nn.Module):
    ...
    def forward(self, inp_img):
        ...
        out_dec_level1 = rearrange(latent, "b (h w) c -> b c h w", h=H//8, w=W//8)
