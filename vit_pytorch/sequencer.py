...
class BiLSTM3D(nn.Module):
    def __init__(self, d_model, hidden_d_model):
        super().__init__()
        self.rnn_v = nn.LSTM(d_model, hidden_d_model, num_layers=1, batch_first=True, bias=True, bidirectional=True)
        self.rnn_h = nn.LSTM(d_model, hidden_d_model, num_layers=1, batch_first=True, bias=True, bidirectional=True)
        self.rnn_z = nn.LSTM(d_model, hidden_d_model, num_layers=1, batch_first=True, bias=True, bidirectional=True)
        self.fc = nn.Linear(6 * hidden_d_model, d_model)
...
class ConvBiLSTM3D(nn.Module):
    def __init__(self, d_model, hidden_d_model):
        super().__init__()
        # self.rnn_v = ConvLSTMCell(d_model, hidden_d_model, kernel_size=3)
        self.conv = nn.Conv3d(d_model, hidden_d_model, kernel_size=3, padding=1, stride=1, bias=False)
        self.rnn_v = ConvLSTMBlock(hidden_d_model, hidden_d_model, kernel_size=3, padding=1, stride=1)
        # self.rnn_h = ConvLSTMBlock(d_model, hidden_d_model, kernel_size=3, padding=1, stride=1)
        # self.rnn_z = nn.LSTM(d_model, hidden_d_model, num_layers=1, batch_first=True, bias=True, bidirectional=True)
        self.fc = nn.Conv3d(hidden_d_model, d_model, kernel_size=1, padding=0, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        # print(x.shape)
        x = self.rnn_v(x.permute(0, 2, 1, 3, 4))
        # print(x.shape)
        # B, S, C, H, W = x.shape
        # x = x.view(B * S, C, H, W)
        x = self.fc(x.permute(0, 2, 1, 3, 4))
        return x
...
class Sequencer3DBlock(nn.Module):
    def __init__(self, d_model, depth, hidden_d_model, expansion_factor=3, dropout=0.):
        ...
        self.model = nn.Sequential(
            *[nn.Sequential(
                PreNormResidual(d_model, nn.Sequential(
                    BiLSTM3D(d_model, hidden_d_model)
                )),
                PreNormResidual(d_model, nn.Sequential(
                    nn.Linear(d_model, d_model * expansion_factor),
                    nn.GELU(),
...
class SequencerConv3DBlock(nn.Module):
    def __init__(self, d_model, depth, hidden_d_model, expansion_factor=3, dropout=0.):
        super().__init__()
        ...
        self.model = nn.Sequential(
            *[nn.Sequential(
                PreNormResidual(d_model, nn.Sequential(
                    ConvBiLSTM3D(d_model, hidden_d_model)
                )),
                PreNormResidual(d_model, nn.Sequential(
                    # nn.Linear(d_model, d_model * expansion_factor),
                    nn.Conv3d(d_model, d_model * expansion_factor, 3, 1, 1, bias=True),
                    nn.GELU(),
                    nn.Dropout(dropout),
                    # nn.Linear(d_model * expansion_factor, d_model),
                    nn.Conv3d(d_model * expansion_factor, d_model, 3, 1, 1, bias=True),
                    nn.Dropout(dropout)
                ))
            ) for _ in range(depth)]
        )
...
class Sequencer2D(nn.Module):
    def __init__(self, model_name: str = 'M', pretrained: str = None, num_classes: int = 16, in_channels=200, out_channel=20, *args, **kwargs) -> None:
        super().__init__()
        ...
        self.patch_embed = SSConv(in_channels, out_channel)
        self.stage = len(depth)
        self.stages = nn.Sequential(
            *[nn.Sequential(
                nn.Conv2d(out_channel if i == 0 else embed_dims[i - 1], embed_dims[i], kernel_size=self.kerner_size[i], padding=self.kerner_size[i]//2, stride=self.patch_size[i]),
...
class SequencerConv3D(nn.Module):
    def __init__(self, model_name: str = 'M', pretrained: str = None, num_classes: int = 16, in_channels=200, out_channel=30, *args, **kwargs) -> None:
        super().__init__()
        assert model_name in sequencer_settings.keys(), f"Sequencer model name should be in {list(sequencer_settings.keys())}"
        depth, embed_dims, hidden_dims, expansion_factor = sequencer_settings[model_name]
        ...
        self.stages = nn.Sequential(
            *[nn.Sequential(
                ...
                SequencerConv3DBlock(embed_dims[i], depth[i], hidden_dims[i], expansion_factor, dropout=0.)
            ) for i in range(self.stage)]
        )
