...
class CotLayer(nn.Module):
    def __init__(self, dim, kernel_size, dw_stride):
        ...
        self.embed = nn.Sequential(
            nn.Conv2d(768, dim // factor, 1, bias=False),
        ...
        self.local_conv = nn.Conv2d(816, dim, kernel_size=1, stride=1, padding=0, dilation=1, bias=False)
...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=200, num_classes=1000, base_channel=64, channel_ratio=6, num_med_block=0,
        ...
        self.trans_patch_conv = nn.Conv2d(128, embed_dim, kernel_size=3, stride=1, padding=1)
        self.trans_1 = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                             qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.trans_dpr[0],
                             )
        # 1 stage combine conv and transformer
        self.trans_conv_1 = CotLayer(embed_dim, kernel_size=3, dw_stride=1)
        self.conv_trans_1 = TranCotLayer(embed_dim, dw_stride=1)
        # self.trans_conv_1 = SimCotLayer(embed_dim, kernel_size=3, dw_stride=1)
        # self.conv_trans_1 = SimTranCotLayer(embed_dim, dw_stride=1)

        # 2 stage

        self.conv_2 = ConvBlock(inplanes=embed_dim, outplanes=embed_dim, res_conv=True, stride=1)
        # self.conv_2 = SSConv(embed_dim, embed_dim, kernel_size=3)
        self.trans_2 = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                             qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.trans_dpr[0],
                             )
        # 2 stage combine conv and transformer
        self.trans_conv_2 = CotLayer(embed_dim, kernel_size=3, dw_stride=1)
        self.conv_trans_2 = TranCotLayer(embed_dim, dw_stride=1)
        # self.trans_conv_2 = SimCotLayer(embed_dim, kernel_size=3, dw_stride=1)
        # self.conv_trans_2 = SimTranCotLayer(embed_dim, dw_stride=1)
        ...
        self.trans_3 = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                             qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.trans_dpr[0],
                             )
        # 3 stage combine conv and transformer
        self.trans_conv_3 = CotLayer(embed_dim, kernel_size=3, dw_stride=1)
        self.conv_trans_3 = TranCotLayer(embed_dim, dw_stride=1)
        ...
        self.trans_4 = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                             qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.trans_dpr[0],
                             )
        # 4 stage combine conv and transformer
        self.trans_conv_4 = CotLayer(embed_dim, kernel_size=3, dw_stride=1)
        self.conv_trans_4 = TranCotLayer(embed_dim, dw_stride=1)

        ## final stage
        self.conv_f = SSConv(embed_dim, embed_dim, kernel_size=3)
        # self.conv_f = ConvBlock(inplanes=embed_dim, outplanes=embed_dim, res_conv=True, stride=1)
        self.trans_f = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                             qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.trans_dpr[0],
                             )
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        # print(x_t.shape)
        # x = self.trans_conv_1(x, x_t)
        # x_t = self.conv_trans_1(x_t[:, 1:], x_raw)
        ...
        # 2 combine
        # x = self.trans_conv_2(x, x_t)
        # x_t = self.conv_trans_2(x_t[:, 1:], x_raw)
        ...
        # 3 combine
        # x = self.trans_conv_3(x, x_t)
        # x_t = self.conv_trans_3(x_t[:, 1:], x_raw)
        ...
        ## final output
        B, C, D, W = x_o.shape
        x_t_o = torch.cat([cls_tokens, x_t_o[:, 1:]], dim=1)
        x_t_o = self.trans_f(x_t_o)
        # x_o = torch.flatten(x_o, start_dim=2, end_dim=3)
        x_t_o = x_t_o[:, 1:].permute(0, 2, 1).reshape(B, C, D, W)
        x_o = self.conv_f(x_o)
        output = torch.cat([x_o, x_t_o], dim=1)
        final_out = self.pooling(output).flatten(1)
        final_cls = self.final_cls_head(final_out)
        # x_o = x_o.permute(0, 2, 1)
        # x_t_o = x_t_o
        # output = torch.cat([x_o, x_t_o], dim=2)
        # output = self.final_trans_pool(output)
        # output = torch.cat([cls_tokens, output], dim=1)
        # output = self.final_trans_norm(output)
        # final_cls = self.final_cls_head(output[:, 0])
        # output = self.outpool(output.permute(0, 2, 1))
        # B, C, D = output.shape
        # output = output.view(B, C*D)
        # # print(output.shape)
