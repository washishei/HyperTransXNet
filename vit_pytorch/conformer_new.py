...
class Conformer(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=200, num_classes=1000, base_channel=64, channel_ratio=6, num_med_block=0,
        ...
        self.conv1_1 = nn.Sequential(
            nn.Conv2d(in_channels=1584, out_channels=64, kernel_size=(3, 3)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
        ...
        self.trans_patch_conv_2 = nn.Sequential(
            # nn.AdaptiveAvgPool2d(1),
        ...
        self.trans_patch_conv_3 = nn.Sequential(
            # nn.AdaptiveAvgPool2d(1),
        ...
        self.conv_f = nn.Sequential(collections.OrderedDict([
            ('conv', nn.Conv2d(256, 256, kernel_size=1, stride=1, padding=0, bias=False)),
            ('bn', nn.BatchNorm2d(256)),
            ('relu', nn.ReLU()),
        ...
        self.trans_patch_conv_f = nn.Sequential(
            # nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(256, 256, kernel_size=1, stride=1, padding=0),
            # nn.AvgPool2d(kernel_size=2, stride=2)
        )
        self.layer_f = LayerNorm(256)
        self.trans_f = Block(dim=256, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                             qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.trans_dpr[0],
                             )
    ...
    def forward(self, x):
        ...
        x_t = self.conv_trans_1(x_t, h_raw)
        ...
        h = self.layer2(h)
        # B, C, D = x_t.shape
        x_t = x_t.permute(0, 2, 1).reshape(B, C, D1, D2)
        ...
        x_t = self.conv_trans_2(x_t, h_raw)
        ...
        # print(h.shape)
        x_t = self.conv_trans_3(x_t, h_raw)
        # x_t_o = x_t
        ...
        ## final output
        x_t_o = self.layer_f(x_t)
        h_o = self.trans_patch_conv_f(h_o)
        output = torch.cat([h_o, x_t_o], dim=1)
        # print(output.shape)
