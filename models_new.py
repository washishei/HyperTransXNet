...
# from vit_pytorch.FeverNet2 import FeverNet
...
from vit_pytorch.vit_SS import ViT_SS
from vit_pytorch.morphFormer import morphFormer
from vit_pytorch.SSFTTnet import SSFTTnet
# from vit_pytorch.FeverNet_Ablation import FeverNet_Ablation
# from vit_pytorch.efficient_Ablation import ViT_Ablation
# from vit_pytorch.FeverNet import FeverNet
...
def get_model(name, **kwargs):
    ...
    #         embed_dims = [168, 168, 512, 512]
    #     layers = [2, 2, 5, 3]
    #     model = FeverNet_Ablation(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=patch_size, transitions=transitions,
    #     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=vit_pytorch.FeverNet_Ablation.ConvPermuteMLP_MixECA, dateset=dataset,)
    #     lr = kwargs.setdefault("learning_rate", 0.0001)
    #     optimizer = optim.Adam(model.parameters(), lr=lr)
    #     criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
    ...
    #     #layers = [4, 3, 6, 3]
    #     # layers = [2, 2, 2, 2]
    #     model = FeverNet(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=patch_size, transitions=transitions,
    #     segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=vit_pytorch.FeverNet.ConvPermuteMLP_MixECA, dateset=dataset,)
    #     lr = kwargs.setdefault("learning_rate", 0.0001)
    #     optimizer = optim.Adam(model.parameters(), lr=lr)
    #     criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
    #     kwargs.setdefault("epoch", 200)
    #     kwargs.setdefault("batch_size", 32)
    #
    # elif name == 'hybridformer':  # patch size 代表模型输入的大小，影响计算量 ##需要注释main中的Summary
    #     patch_size = kwargs.setdefault('patch_size', 15)  # 19\15\11
    #     center_pixel = True
    #     d_h = [4, 2]
    #     model = ViT_SS(image_size=patch_size, patch_size=[3, 5], num_classes=n_classes, dim=100, depth=4, heads=2,
    ...
    #     kwargs.setdefault('batch_size', 32)
        ...
        optimizer = optim.Adadelta(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
    elif name == 'cnn2D':##fevernet:用这个2dcnn##
        patch_size = kwargs.setdefault('patch_size', 15)  # default = 11
        center_pixel = True
        model = Cnn2D(n_bands, n_classes, patch_size=patch_size)
        lr = kwargs.setdefault('learning_rate', 0.0001)  # 设置的0.001
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault('batch_size', 32)  # 30 samples=128,10 samples=64
        kwargs.setdefault('epoch', 200)  # default 100
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    else:
...
class HybridEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=15):
        ...
        self.conv4 = nn.Sequential(
            ...
            nn.ReLU()
        ) ## 6016 for IN, 4224 for Grss ##2912 for PU
...
def t_SNE(
    ...
    for batch_idx, (data, target) in tqdm(
        ...
        target_layer = net.norm  # DCTN,HiT,SS_TMNet
...
def test2(net, img, hyperparams):
    ...
    for batch in tqdm(
        ...
        with torch.no_grad():
            ...
            # add
            # intermediate_features = net.fc_1.weight.data.cpu().numpy()  # 这里示例提取全连接层 'fc1' 的权重
            # features.append(intermediate_features)
            # 定义一个钩子函数，用于捕获 self.fc_1 层的输出
            activation = {}  # 用于存储捕获的输出
            ...
            output = net(data)
            # 获取捕获的 self.fc_1 层的输出
            fc_1_output = activation['fc_1_output']
            fc_1_output = fc_1_output.mean(1)#DCTN
            # print(fc_1_output.shape)#torch.Size([32, 512])
            features.append(fc_1_output.cpu().detach().numpy())
    ...
    #print(features.shape)#(194296, 1024)
    #end
    end.record()
    torch.cuda.synchronize()
    print("The testing time is:***********************", start.elapsed_time(end)/1000)
