...
from vit_pytorch.SS_TMNet import SS_TMNet, SSAM
...
from vit_pytorch.DCTN import DCTN, EISA
from vit_pytorch.morphFormer import morphFormer
from vit_pytorch.SSLANet import SSLANet
...
from vit_pytorch.gscvit import GSCViT
# from vit_pytorch.TIMN  import TiMN
from UMMA.UMMA_main import UMMA
# from mamba_pytorch.TMamba2D import TMamba2D
# from mamba_pytorch.mambaT import TMamba2D
from kan_convolutional.kcvt import KCvT
from kan_convolutional.KANConv3D import KC3DEtAl
from kan_convolutional.KANConv2D import KC2DEtAl
from kan_convolutional.KANLinear import KC1DEtAl
from kan_convolutional.hybridkan import HybridKNet
from Retnet.retnet1d import RetNet as Retnet1D
from Retnet.retnet2d import VisRetNet as Retnet2D
# from Retnet.retnet3d import VisRetNet3D as Retnet3D
from Retnet.retnet3dn import RetNet3D as RMT3DN
from Retnet.rmt3d import RMT3D as Retnet3D
from Retnet.hybridretnet import VisRetNetNew as HRetnet
# from mamba_pytorch.MiM_v3 import MiM_v3
# from mamba_pytorch.MiM_v2 import MiM_v2
from mamba_pytorch.MiM_v1 import MiM_v1
from kan_convolutional.CATT import CAT
from vit_pytorch.HyperTransXNet import HyperSpectralClassifier as HyperTransXNet
# from vit_pytorch.mamba3DN import Light3DTransformer as ICCmamaba

# from RL.RLEhancedViT import HyperSpectralViT as RLT
from vit_pytorch.CATTT import CvT as CATT
# from kan_convolutional.CVKATT import CvKAT
from mamba_pytorch.harm3D import TMamba3D
from mamba_pytorch.mamba3D import TMamba3DD
from mamba_pytorch.HNN import HTESTEtAl
from mamba_pytorch.MambaHSI import MambaHSI
from mamba_pytorch.HyperMamba import HyperMamba
from mamba_pytorch.TDSSMamba import ThreeDSSMamba
from mamba_pytorch.QMambaNet import QMambaNet
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        model = CAT(num_classes=n_classes, channels=n_bands)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adadelta(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    ...
    #     )
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = DKAN2DCNN(input_channels=n_bands, n_classes=n_classes,)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = HyperKAN(in_channels=n_bands, num_classes=n_classes,)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = SSLANet(in_channels=n_bands, num_classes=n_classes,)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = HyperTransXNet(input_dim=n_bands, num_classes=n_classes, reduce_dim=30, num_blocks=4)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = ICCmamaba(in_channels=n_bands, num_classes=n_classes,)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
    ...
    #     kwargs.setdefault("patch_size", 15)
    #     center_pixel = True
    #     model = RLT(in_channels=n_bands, num_classes=n_classes)
    #     lr = kwargs.setdefault("learning_rate", 0.0001)
    #     optimizer = optim.Adam(model.parameters(), lr=lr)
    #     criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
    #     kwargs.setdefault("epoch", 100)
    #     kwargs.setdefault("batch_size", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = HTESTEtAl(n_bands, n_classes)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        center_pixel = True
        model = TMamba3DD(in_channels=n_bands, num_classes=n_classes)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        model = UMMA(padding_val=0, input_dim=225, embd_dim=256, n_classes=n_classes,)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adadelta(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        ...
        # default of  0.002 to  train the  network"
        lr = kwargs.setdefault('lr', 0.8)
        model = HyperMamba(in_channels=n_bands, num_classes=n_classes,)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adadelta(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        ...
        # default of  0.002 to  train the  network"
        lr = kwargs.setdefault('lr', 0.001)
        model = ThreeDSSMamba(in_channels=n_bands, num_classes=n_classes,)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adadelta(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        ...
        # default of  0.002 to  train the  network"
        lr = kwargs.setdefault('lr', 0.1)
        model = QMambaNet(in_channels=n_bands, num_classes=n_classes,)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adadelta(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        ...
        kwargs.setdefault("patch_size", 16)
        center_pixel = True
        model = TMamba3D(in_channels=n_bands, num_classes=n_classes)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 1)
        center_pixel = True
        model = Retnet1D(num_class=n_classes, channels=n_bands, layers=12, hidden_dim=128, ffn_size=1024, heads=4, double_v_dim=True)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("epoch", 100)
        kwargs.setdefault("batch_size", 100)
    elif name == "hybridretnet":
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = HRetnet(
        in_chans = n_bands,
        num_classes=n_classes,
        embed_dims=[256, 512, 512, 256],
        depths=[2, 3, 8, 4],
        num_heads=[4, 4, 4, 4],
        init_values=[2, 2, 2, 2],
        heads_ranges=[4, 4, 6, 6],
        ...
        kwargs.setdefault("patch_size", 16)
        center_pixel = True
        model = RMT3DN(in_chans=1, num_classes=n_classes)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
    ...
    #     kwargs.setdefault("patch_size", 15)
    #     center_pixel = True
    #     model = CvKAT(num_classes=n_classes, channels=n_bands,)
    #     lr = kwargs.setdefault("learning_rate", 0.001)
    #     optimizer = optim.Adam(model.parameters(), lr=lr)
    #     criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
    #     kwargs.setdefault("epoch", 100)
    #     kwargs.setdefault("batch_size", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = KCvT(num_classes=n_classes, channels=n_bands,)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = KC3DEtAl(n_bands, n_classes)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = KC2DEtAl(n_bands, n_classes)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = KC1DEtAl(n_bands, n_classes)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
    ...
    #     kwargs.setdefault("patch_size", 15)
    #     center_pixel = True
    #     model = TiMN(num_classes=n_classes, channels=n_bands,)
    #     lr = kwargs.setdefault("learning_rate", 0.0001)
    #     optimizer = optim.Adam(model.parameters(), lr=lr)
    #     criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
    #     kwargs.setdefault("epoch", 100)
    #     kwargs.setdefault("batch_size", 100)
        ...
        segment_dim = [8, 8, 4, 4]
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [544, 544, 256, 256]## for IN 368, for GRSS 256, for PU 168, for KSC 320 for XA 480
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = HybridKNet(n_bands, n_classes, patch_size=15)
        lr = kwargs.setdefault("learning_rate", 0.001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
...
##t-SNE可视化
    ...
    embeddings = TSNE(n_components=2,perplexity=10, learning_rate=50,init='pca', random_state=0).fit_transform(#perplexity=400, learning_rate=50, n_jobs=8
    ...
    plt.savefig('/root/project/DeepTy/t_sne/IndianPines/hybridsn_tsne', dpi=600)
    plt.show()
    exit()
