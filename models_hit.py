...
from vit_pytorch.hit_linear import HiTLINEAR, ConvPermuteMLP
from vit_pytorch.hit_conv2d import HiTCONV2D, ConvPermuteMLP
from vit_pytorch.hit_conv3d import HiTCONV3D, ConvPermuteMLP
...
def get_model(name, **kwargs):
    ...
    #     kwargs.setdefault("batch_size", 100)
        ...
        segment_dim = [4, 4, 4, 4]
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [208, 208, 512, 512]## for IN 368, for GRSS 256, for PU 168, for KSC 320 for XA 480
        ...
        model = HiTLINEAR(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3, transitions=transitions,
        segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=ConvPermuteMLP,)
        lr = kwargs.setdefault("learning_rate", 0.0001)## for KSC 0.000003
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [368, 368, 512, 512]## for IN 368, for GRSS 256, for PU 168, for KSC 320 for XA 480
        model = HiTCONV2D(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3, transitions=transitions,
        segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=ConvPermuteMLP,)
        lr = kwargs.setdefault("learning_rate", 0.0001)## for KSC 0.000003
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        ...
        segment_dim = [4, 4, 4, 4]
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [168, 168, 512, 512]## for IN 368, for GRSS 256, for PU 168, for KSC 320 for XA 480
        model = HiTCONV3D(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3, transitions=transitions,
        segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=ConvPermuteMLP,)
        lr = kwargs.setdefault("learning_rate", 0.0001)## for KSC 0.000003
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
