...
from vit_pytorch.qtnn import QTNN, SpatialAttention
...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = SSCDNonLModel(n_bands=n_bands, num_classes=n_classes, chanel=150)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
        ...
        model = QtN(dim=128, image_size=15, patch_size=3, depth=4, heads=4, dropout=0.1, emb_dropout=0.1, num_classes=n_classes, channels=n_bands,)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adadelta(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        ...
        segment_dim = [8, 8, 4, 4]
        mlp_ratios = [3, 3, 3, 3]
        embed_dims = [128, 128, 256, 256]## for IN 368, for GRSS 256, for PU 168, for KSC 320 for XA 480
        model = QTNN(layers, img_size=15, in_chans=n_bands, num_classes=n_classes, embed_dims=embed_dims, patch_size=3, transitions=transitions,
        segment_dim=segment_dim, mlp_ratios=mlp_ratios, mlp_fn=SpatialAttention,)
        ...
        kwargs.setdefault("patch_size", 15)
        center_pixel = True
        model = Conformer(patch_size=15, channel_ratio=6, embed_dim=384, depth=12,
                      num_heads=12, mlp_ratio=4, qkv_bias=True, num_classes=n_classes, in_chans=n_bands)
        lr = kwargs.setdefault("learning_rate", 0.0001)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
        kwargs.setdefault("epoch", 100)
...
class SSCDNonLModel(nn.Module):
    def __init__(self, num_classes, n_bands, chanel):
        ...
        self.flatten = nn.Flatten()
        self.fc = nn.Linear(4050, num_classes)
